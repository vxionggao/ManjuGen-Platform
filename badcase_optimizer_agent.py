...
app_name = "badcase_optimizer_app"

agent_name = "漫剧Badcase优化Agent"
description = "漫剧创作平台Badcase优化智能体，具备技能能力"
    ...
    try:
        ...
        if hasattr(response, 'content') and response.content:
            if hasattr(response.content, 'parts') and response.content.parts:
                answer = "".join([part.text for part in response.content.parts if hasattr(part, 'text') and part.text])
                return {"answer": answer}
        
        # 如果无法提取内容，返回原始响应
        return {"answer": str(response)}
    except Exception as e:
        logger.exception("Error in agent run: %s", e)
        return {"error": str(e)}
